...
reflectance_files = sorted(Path(f'/home/kyle/iuvs/reflectance/orbit0{block}').glob('*nonlinear*'))
reflectance = np.load(reflectance_files[file]) / ff

# Load in the corrected wavelengths
# TODO: these are the linear ones, not the nonlinear ones
wavelengths_files = sorted(Path(f'/home/kyle/iuvs/wavelengths/orbit0{block}').glob(f'mvn_iuv_wl_apoapse-orbit0{orbit}-muv*.sav'))
...
def retrieval(integration: int, spatial_bin: int):
    ...
    # TODO: this now gives approximately the same value in each grid. Is this right?
    colden = column_density(linear_grid_profile, linear_grid_profile, z, (z, pixel_pressure), (z, pixel_temperature))[:first_bad]  # flip so TOA is first
    ...
    wolff_hapke = np.interp(pixel_wavs, np.linspace(0.258, 0.32, num=100), np.linspace(0.07, 0.095, num=100))
    ...
    for counter, foobar in enumerate(wolff_hapke):
        brdf_arg = np.array([0.8, 0.06, foobar, 0.3, 0.45, 20])
    ...
    def simulate_tau(guess: np.ndarray) -> np.ndarray:
        ...
        for counter, wav_index in enumerate(wavelength_indices):
            ...
            dust_extinction = pyrt.extinction_ratio_grid(dust_cext, dust_particle_sizes, dust_wavelengths, 0.24)
            dust_reff_grid = np.linspace(1.5, 1.5, num=100)
            dust_z_reff = np.linspace(100, 0, num=100)
            dust_reff = np.interp(z[:first_bad], dust_z_reff, dust_reff_grid)   # The "bad" altitudes don't get assigned a particle size
            ...
            ##############
            # Before: pixel_wavs[wav_index]. Now I'm scaling it to 240 nm
            ice_extinction = pyrt.extinction_ratio_grid(ice_cext, ice_particle_sizes, ice_wavelengths, 0.24)
            ice_reff_grid = np.linspace(3, 3, num=100)
            ice_z_reff = np.linspace(100, 0, num=100)
            ice_reff = np.interp(z[:first_bad], ice_z_reff, ice_reff_grid)   # The "bad" altitudes don't get assigned a particle size
            ...
            rfldir, rfldn, flup, dfdt, uavg, uu, albmed, trnmed = \
                disort.disort(True, False, False, False, [False, False, False, False, False],
                              ...
                              np.pi, 0, wolff_hapke[wav_index], 0, 0, 1, 3400000, h_lyr,
    ...
    fitted_optical_depth = minimize(find_best_fit, np.array([0.4, 0.3]), method='Nelder-Mead', bounds=((0, 0.8), (0, 1.5))).x
...
np.save(f'/home/kyle/iuvs/retrievals/orbit03400/orbit{orbit}-{file}-dust-updateAug1-hg2real-lasp.npy', retrieved_dust)
np.save(f'/home/kyle/iuvs/retrievals/orbit03400/orbit{orbit}-{file}-ice-updateAug1-hg2real-lasp.npy', retrieved_ice)
np.save(f'/home/kyle/iuvs/retrievals/orbit03400/orbit{orbit}-{file}-chi_squared-updateAug1-hg2real-lasp.npy', retrieved_chi_squared)
t1 = time.time()
print(t1-t0)
